...
function fmtNum(val: number | null, decimals: number = 0): string {
  if (val == null) return '';
...
export function formatTriangle(triangle: Triangle): string {
  ...
  for (let i = 0; i < accident_years.length; i++) {
    const row =
      pad(String(accident_years[i]), labelWidth, 'left') +
...
export function formatAgeToAgeFactors(
  ...
  lines.push(
    pad(selLabel, labelWidth, 'left') +
    factors.selected.map((f) => pad(fmtNum(f, 4), colWidth)).join('')
  );
  lines.push(
    pad('CDF', labelWidth, 'left') +
...
export function formatAgeToAgeFactorsCompact(
  factors: AgeToAgeFactors,
  accidentYears?: number[],
  showLabels?: string[]
): string {
  ...
  const labelsToShow = showLabels ?? defaultLabels;
...
export function formatTrendFits(results: TrendFitResult[], title: string = 'Trend Fits'): string {
  const lines: string[] = [];
  lines.push(title);
  lines.push('');
  lines.push(
...
export function formatIndicationSummary(summary: IndicationSummary): string {
  const lines: string[] = [];
